...
## Mission
Build **data sources** from a clean student workspace. You will create/edit files, execute commands, validate independently, break the configuration, fix it, complete a challenge, compare the solution and clean up.
...
## 2. Enter the lab
```powershell
cd data2ai-terraform-azure-foundry/02-terraform-foundations/lab-02-03-data-sources
Get-ChildItem -Force
```
...
az group show --name rg-data2ai-02-03-lab --query "{name:name,location:location,tags:tags}" --output json
```
```powershell
terraform output
